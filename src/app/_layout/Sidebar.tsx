    ...
    return (
      <div className="relative mx-auto w-full max-w-2xl p-5 pb-20">
        <div className="flex flex-col space-x-0 sm:flex-row sm:space-x-4">
          {sidebarItems.map((section) => (
            <div className="w-full sm:w-1/2" key={section.title}>
        ...
        <a
          ...
          className="absolute bottom-6 right-[84px]"
        >
          <Icon
            icon="fa6-solid:hand-peace"
        ...
        <a href={siteConfig.links.github} className="absolute bottom-6 right-6">
          <Icon
            icon="fa-brands:github"
            className="size-8 text-white hover:text-neutral-500"
          />
  ...
  function SidebarHeader({
    ...
    return (
      <span
        ...
        } group rounded-lg font-semibold text-white`}
      >
        {title}
      </span>
    )
